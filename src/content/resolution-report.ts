...
const RESOLUTION_DIR = join(REPOSITORY_ROOT, 'resolution');
...
/** `{ 'resolution/foo.notation.json': '…', … }` — the full expected tree. */
export function buildResolutionReports(
  ...
  for (const lesson of manifest.lessons) {
    files.set(
      `resolution/${lesson.id}.notation.json`,
      stableJson({
...
export function syncResolutionReports(
  ...
  for (const [relPath, content] of [...expected].sort(([a], [b]) =>
    ...
    const name = relPath.slice('resolution/'.length);
  ...
  for (const orphan of [...onDisk].sort()) {
    ...
    stale.push(`resolution/${orphan} (stale, removed)`);
  }
