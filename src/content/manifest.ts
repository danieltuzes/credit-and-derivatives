...
export async function compileManifest(): Promise<Manifest> {
...
/**
 * The reader every consumer uses. Reads the `.generated` file when it exists;
 * otherwise compiles and writes it once (a cold `astro dev`). Normal
 * `pnpm build` / `pnpm verify` regenerate it in `validate:content` first, so
 * this only compiles on a bare checkout.
 */
