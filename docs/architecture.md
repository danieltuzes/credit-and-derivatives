...
`packages/legend/` (published as `@danieltuzes/legend`, built with `tsup` to
`packages/legend/dist/`); the **course** is the repo root (`content/`,
`astro.config.mjs`, `scripts/`, `tests/`), depending on the engine via
`workspace:*`. Engine paths written `src/reference/…`, `src/compiler/…`,
`src/components/…` below are `packages/legend/src/…`; the course imports them as
`@danieltuzes/legend/reference/…` etc. `src/content.config.ts` at the root is a
...
| Engine / course split           | pnpm workspace: the engine is `packages/legend/` (`@danieltuzes/legend`, `tsup` build → `dist/` + `.d.ts`), the course is the repo root and depends on it via `workspace:*`; the compiler takes the course config as a parameter and names no content path (F1 detangled, F2 packaged) | `packages/legend/package.json`, `content/course.config.ts`, `tests/unit/boundaries.test.ts`                                  | ✅     |
...
`@danieltuzes/legend`) is complete. What remains:
...
- **Split into two repos.** The engine (`packages/legend/`) and the course
  (repo root) are a pnpm workspace bound by `workspace:*`; the intended end
  state is `@danieltuzes/legend` on npm and a separate `credit-and-derivatives`
...
`@danieltuzes/legend`'s `compiler/manifest.ts` `compileManifest(courseConfig)` walks `content/` **once**
