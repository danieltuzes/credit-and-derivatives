...
**Layout (Phase F2).** The **engine** is its own package and repo,
[`explico`](https://github.com/danieltuzes/explico) (built with `tsup` to
`dist/` + `.d.ts`, published to npm); **this repo is the course** — the repo
root (`content/`, `astro.config.mjs`, `scripts/`, `tests/`) — and depends on
`explico` as a pinned version. Engine paths written `src/reference/…`,
`src/compiler/…`, `src/components/…` below are the engine's own `src/…`; the
course imports them as `explico/reference/…` etc. `src/content.config.ts` at
the root is a one-line re-export of the engine's schemas.
...
| Keyed tables and figures        | Numbered `<Table>` / `<Figure>` floats with stable anchors, cross-page references, preview, and deep-link focus                                                                                                                                                                                                                             | `src/components/{table,figure,reference}/*`, `src/reference/{tables,figures,keyed-float}-validate.ts`                        | ✅     |
| Static diagrams                 | Build-time Dagre layout for accessible graphs and flowcharts, with optional notation-bound KaTeX node labels                                                                                                                                                                                                                                | `src/components/diagram/Diagram.astro`, `src/reference/diagram-layout.ts`                                                    | ✅     |
| About and knowledge version     | Header `[?]` panel with course identity, authorship, disclaimer, engine version, and changelog-backed knowledge version                                                                                                                                                                                                                     | `content/course.config.ts`, `content/CHANGELOG.md`, `src/components/starlight/AboutPanel.astro`                              | ✅     |
...
| Engine / course split           | the engine is the separate [`explico`](https://github.com/danieltuzes/explico) package (`tsup` build → `dist/` + `.d.ts`, on npm); this repo is the course and depends on a pinned `explico` version; the compiler takes the course config as a parameter and names no content path (F1 detangled, F2 packaged, F2-remote split into repos) | `package.json` (`explico` dep), `content/course.config.ts`, `tests/unit/boundaries.test.ts`                                  | ✅     |
...
### Equation identity and keyed floats (D7, F3)
...
An author marks a referenced equation with a trailing `\label{eq:<key>}`
...
An authored `id="<key>"` creates `#tab-<key>` / `#fig-<key>`; prose refers to
it with `[[tab-<key>]]` / `[[fig-<key>]]`, including cross-lesson slug forms.
`FloatRefLayer` and `KeyedFloatEnhancer` provide one preview, pin, jump, and
focus engine for equations, tables, and figures.
...
`scroll-margin` for the sticky header. `KeyedFloatEnhancer.astro` (lessons only)
...
Tier-3 warning. Parallel table and figure validators emit corresponding
`tab-*` / `fig-*` diagnostics. The manifest records every keyed float and its
rendered number.
...
(lessons only) mirrors `KeyedFloatEnhancer` — repeat-activation re-trigger, smooth
...
| Content semantics | `tsx scripts/compile-manifest.ts` | `src/curriculum/validation.ts`, `src/reference/{registry,gate-math,consistency,equations-validate,keyed-float-validate,curriculum-alignment}` | Invalid/duplicate IDs; unknown references; prerequisite and ordering defects; assessment coverage; citation/source drift; review-state defects; notation and alignment defects; unresolved identifiers; equation/table/figure reference defects; knowledge-version drift; corpus consistency warnings; stale committed resolution reports |
...
| Every `[[tab-key]]` / `[[fig-key]]` resolves; keyed tables and figures are unique per lesson                               | `reference/{tables,figures,keyed-float}-validate.ts`; `remark-notation` on the render path                                                  |
| Graphs and flowcharts use one deterministic, no-JS layout primitive                                                        | **guideline-only** — `components/diagram/Diagram.astro` is the approved primitive                                                           |
...
| Notation   | `tests/notation/*`   | Vitest                              | registry, compiler, `remark-notation`, `remark-citation`, `math-glyphs`, `gate-math`, equations and keyed floats, consistency, KaTeX options, curriculum alignment, file loaders, lab/notation math rendering, manifest determinism |
...
- **Engine unit tests.** Tests that exercise engine internals with synthetic
  fixtures (e.g. the notation-math renderer's trust boundary, the gloss
  resolver) belong in the [`explico`](https://github.com/danieltuzes/explico)
  repo alongside the code they guard; the split moved the course-integration
  tests here and left the engine repo's own suite to be filled in.
...
### Shape (`schemaVersion` 6)

| Field                                 | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `schemaVersion`                       | Integer; a client asserts against it before trusting the file.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `about`                               | Engine identity plus the configured course material identity, authors, disclaimer, changelog link, and resolved knowledge version.                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `equations` / `tables` / `figures`    | `{ labels[], numbersBySlug }`: every keyed float with its chapter-scoped number and the cross-page lookup table read by `remarkNotation`.                                                                                                                                                                                                                                                                                                                                                                                                     |
| `diagnostics`                         | `{ curriculum, notation, alignment, math, consistency, knowledge, equations, tables, figures }` — every issue from every pass, blocking or not.                                                                                                                                                                                                                                                                                                                                                                                               |
