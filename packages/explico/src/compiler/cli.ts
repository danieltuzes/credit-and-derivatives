...
// Content and its course math are the top-level `content/` folder (Phase F).
const CONTENT_ROOT = join(REPOSITORY_ROOT, 'content');
const DOCS_ROOT = join(CONTENT_ROOT, 'docs');
const DOMAIN_ROOT = join(CONTENT_ROOT, 'domain');
...
function mentions(message: string, lessonId: string, file: string): boolean {
  return (
    ...
    message.includes(file.replace(/^content\/docs\//, ''))
  );
}
...
/**
 * Which `content/domain/` folders a lesson section draws on. A pragmatic map: most
...
export function changedLessonIds(): string[] {
  ...
  try {
    ...
    const untracked = execFileSync(
      'git',
      ['ls-files', '--others', '--exclude-standard', '--', 'content/docs'],
      { cwd: REPOSITORY_ROOT, encoding: 'utf8' },
    );
    output = `${tracked}\n${untracked}`;
  } catch {
...
export function scaffoldLesson(rawId: string): Scaffold {
  ...
  const relPath = `content/docs/${slug}.mdx`;
  const checksPath = `content/docs/${slug}.checks.yml`;
...
<!-- the reproducible worked example; every numeral tagged to a content/domain/ call or [@source; locator] -->
`;
  ...
  return {
    ...
    companions: [
      {
        path: checksPath,
        contents: `# Assessment sets for this lesson. Ids resolve against content/assessments/.\n[]\n`,
      },
    ],
  };
}
...
export function scaffoldTerm(rawKey: string): Scaffold {
  ...
  const relPath = `content/notation/${key}.md`;
  if (existsSync(join(REPOSITORY_ROOT, relPath))) {
    throw new ContentCliError(
      'exists',
      `${relPath} already exists; not overwriting.`,
